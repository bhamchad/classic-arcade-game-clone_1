...
var Enemy = function(X, Y, V) {
    ...
    this.sprite = 'images/enemy-thors-hammer.png';
...
var Player = function(X, Y) {
    ...
    this.sprite = 'images/char-loki.png';
};
...
function renderScore(obj) {
    ...
    ctx.fillStyle = '#208f42';
    ctx.font = "30px Arial";
    ctx.textAlign = "left";
    ctx.fillText("Score: " + player.score, 0, 40);
    ctx.textAlign = "right";
    ctx.fillText("Loki's left: " + player.lives, 505, 40);
};
...
function checkCollision (X, Y, arrayObjs) {
    for (obj in arrayObjs) {
        ...
        //to check the collision, first I checked in which square is my player and if one of the enemy is there or at least entering there
...
function restartPlayer(obj) {
    ...
    if (obj.lives <= 0) {
        //finish game, stop engine
        alert("Thor's hammer is too powerful! Game Over");
        obj.score = 0;
        obj.lives = 3;
...
/**
 * Gems's function to randomly allocate on the game board.
 */
...
/**
 * Gems's function to specify how long they appear in the game.
 */
...
/**
 * Gems's function to update the gem's position.
 */
...
/**
 * Function to gen random int between two ints.
 * @param {int} min is an int (lower bound).
 * @param {int} max is an int (upper bound).
 * @return {int} This return a random int.
 */
